...
try {
  ...
  const pp = page.locator('[data-practice="0"]');
  await pp.fill('8');
  ...
  await page.getByText('멤버 자료 관리', { exact: true }).click();
  await room('stage');
  const beforePlaybackCalls = requests.length;
  await page.getByRole('button', { name: '완성된 무대 재생', exact: true }).click();
  ...
  const wav = readFileSync(join(dir, 'completed-song.wav'));
  ...
  assert.equal(await page.locator('#window-body').evaluate(el => el.scrollWidth <= el.clientWidth), true);
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), true);
  await page.keyboard.press('Escape');
  assert.equal(await page.locator('#game-window').evaluate(el=>el.open), false);
