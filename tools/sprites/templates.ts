...
/** 검정 세부선 역할(초커·안경테): 외곽선 K 와 같은 색이지만 음영이 '특징'으로 취급해 선택적 외곽선에서 검정을 지킨다. */
export const DETAIL_LINE = 'k';
...
/** 지정한 행의 role 칸을 어둠 톤 문자로 바꾼다(뒷머리 끝단 등). 톤 문자는 규칙 음영이 건드리지 않는다. */
export function darkRows(g: Grid, role: string, rowsAt: number[]): Grid {
  const d = variantChar(role, 'dark');
  return g.map((r, y) => (rowsAt.includes(y) ? r.replaceAll(role, d) : r));
}
/** 앞머리 갈래 끝 1px 어둠: 안쪽(x0~x1)·rows 범위에서 바로 아래가 role 이 아닌 role 칸. */
export function tipDark(g: Grid, role: string, y0: number, y1: number, x0: number, x1: number): Grid {
  const d = variantChar(role, 'dark');
  return g.map((r, y) => {
    if (y < y0 || y > y1) return r;
    const row = [...r];
    for (let x = x0; x <= x1; x++) if (row[x] === role && g[y + 1]?.[x] !== role) row[x] = d;
    return row.join('');
  });
}
/**
 * 정수리 광택(손 배치): spans 안에서 role 인 칸만 밝음 톤으로 바꾼다.
 * 라이브러리의 자동 광택은 런의 왼쪽 65%를 통째로 밝게 해 16×16 목숨 아이콘에서 네모난 판으로 보인다 —
 * 캐릭터 음영에서는 그 옵션을 끄고(build-sprites-lib) 여기서 모양을 잡는다.
 */
export function sheen(g: Grid, role: string, spans: Span[]): Grid {
  const l = variantChar(role, 'light');
  const w = g[0]?.length ?? SPRITE_W;
  const mask = fill('#', spans, w, g.length);
  return g.map((r, y) => [...r].map((ch, x) => (ch === role && mask[y]![x] === '#' ? l : ch)).join(''));
}
/** 머리 위 광택 띠(3~6행). 빛이 왼쪽 위에서 오므로 왼쪽으로 치우치고 아래로 갈수록 좁아진다. 앞머리 캡과 뒤통수가 같은 타원이라 둘 다 쓴다. */
export const HEAD_SHEEN: Span[] = [[3, 3, 11, 17], [4, 4, 10, 18], [5, 5, 10, 16], [6, 6, 11, 13]];
...
/** 뒷모습용 머리 뒤통수: 얼굴 타원을 앞머리색으로 채우고 앞모습과 같은 정수리 광택을 얹는다. */
export const HEAD_BACK: Grid = sheen(FACE.slice(0, 23).map((r) => r.replace(/S/g, 'H')), 'H', HEAD_SHEEN);
...
/** 앞머리 마감: 갈래 끝 1px 어둠(7~12행, 옆머리 제외한 안쪽 6~25열) + 정수리 광택. */
const bangs = (g: Grid): Grid => sheen(tipDark(g, 'H', 7, 12, 6, 25), 'H', HEAD_SHEEN);
export const HAIR: Record<HairStyle, { front: Grid; back: Grid }> = {
  // 원이: 어깨 아래 긴 생머리, 일자 앞머리
  long: {
    front: bangs(punch(outlined(fill('H', [...CAP, [7, 10, 5, 26], [8, 24, 4, 5], [8, 24, 26, 27]])), [[10, 9], [10, 13], [10, 18], [10, 22], [11, 9], [11, 22]])),
    back: darkRows(outlined(fill('h', [[4, 5, 6, 25], [6, 8, 4, 27], [9, 34, 3, 28], [35, 38, 4, 27], [39, 40, 5, 26], [41, 41, 6, 7], [41, 41, 24, 25]])), 'h', [40, 41]),
  },
  // 리브: 턱선 단발, 안으로 말린 끝, 옆으로 흘린 앞머리
  bob: {
    front: bangs(punch(outlined(fill('H', [...CAP, [7, 9, 5, 26], [10, 10, 5, 12], [10, 10, 22, 26], [11, 11, 5, 8], [8, 19, 4, 5], [8, 19, 26, 27], [20, 20, 5, 6], [20, 20, 25, 26], [21, 21, 6, 6], [21, 21, 25, 25]])), [[9, 15], [9, 19], [10, 11], [10, 25]])),
    back: outlined(fill('h', [[4, 5, 6, 25], [6, 8, 4, 27], [9, 21, 3, 28], [22, 22, 4, 27], [23, 23, 5, 26]])),
  },
  // 미나미: 물결 롱헤어, 가르마 없는 옆으로 넘긴 앞머리, 끝이 퍼진다
  wavy: {
    front: bangs(punch(outlined(fill('H', [...CAP, [7, 9, 5, 26], [10, 10, 5, 9], [10, 10, 19, 26], [11, 11, 5, 6], [11, 11, 22, 26], [12, 12, 24, 26], [8, 13, 4, 5], [8, 13, 26, 27], [14, 16, 3, 5], [14, 16, 26, 28], [17, 19, 4, 5], [17, 19, 26, 27], [20, 24, 3, 5], [20, 24, 26, 28]])), [[10, 21], [11, 24]])),
    back: darkRows(outlined(fill('h', [[4, 5, 6, 25], [6, 8, 4, 27], [9, 12, 3, 28], [13, 16, 2, 29], [17, 20, 3, 28], [21, 24, 2, 29], [25, 28, 3, 28], [29, 32, 2, 29], [33, 36, 1, 30], [37, 40, 2, 29], [41, 42, 1, 30], [43, 43, 2, 4], [43, 43, 27, 29]])), 'h', [42, 43]),
  },
  // 메이: 귀 위 짧은 단발, 삐죽한 앞머리
  short: {
    front: bangs(punch(outlined(fill('H', [...CAP, [7, 9, 5, 26], [10, 10, 5, 26], [8, 14, 4, 5], [8, 14, 26, 27], [15, 15, 5, 5], [15, 15, 26, 26]])), [[10, 8], [10, 12], [10, 16], [10, 20], [10, 24], [11, 8], [11, 16], [11, 24]])),
    back: outlined(fill('h', [[4, 5, 6, 25], [6, 8, 4, 27], [9, 15, 3, 28], [16, 16, 4, 27]])),
  },
  // 제나: 트윈테일 + 리본(신라공주), 가운데 살짝 갈라진 앞머리
  twin: {
    front: bangs(punch(outlined(fill('H', [...CAP, [7, 10, 5, 26], [8, 11, 4, 5], [8, 11, 26, 27]])), [[10, 15], [10, 16], [9, 15], [9, 16], [11, 15], [11, 16], [10, 9], [10, 22]])),
    back: darkRows(outlined(fill('h', [[4, 5, 6, 25], [6, 8, 4, 27], [9, 10, 3, 28], [8, 12, 1, 3], [8, 12, 28, 30], [13, 30, 0, 3], [13, 30, 28, 31], [31, 35, 1, 3], [31, 35, 28, 30], [36, 37, 0, 2], [36, 37, 29, 31]])), 'h', [36, 37]),
  },
};
...
export const ACCESSORIES: Record<Accessory, Grid> = {
  ...
  choker: fill(DETAIL_LINE, [[24, 24, 12, 19]]),                           // 초커 검정 1행(세부선: 선택적 외곽선에서 유지)
...
export const BASE_PALETTE: Record<string, string> = {
  K: '#12131c', k: '#12131c', S: '#f2cfb3', s: '#f0a3a3', E: '#12131c', W: '#ffffff', m: '#c8465f', p: '#ff8fb1',
