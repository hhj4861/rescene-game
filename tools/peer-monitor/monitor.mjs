...
export class Receiver {
  ...
  async tick() {
    ...
    try {
      ...
      if (fresh.length) {
        try { await this.notify(fresh.length); this.state.notificationCommand = { at: new Date().toISOString(), status: 'submitted' }; }
...
export async function start(config) {
  ...
  const server = createServer((req, res) => {
    ...
    const routes = { '/': ['text/html', page], '/app.js': ['text/javascript', script], '/api/inbox': ['application/json', JSON.stringify(receiver.state)] };
  ...
  await receiver.tick();
  const timer = setInterval(() => receiver.tick(), 10000);
