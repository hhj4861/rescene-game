...
import { ActionQueue, createExecutor, createDelivery } from './actions.mjs';
...
export async function start(config) {
  ...
  const actions = config.actions?.enabled ? new ActionQueue(config, createExecutor(config), createDelivery(config)) : null;
  ...
  const server = createServer((req, res) => {
    ...
    const routes = { '/': ['text/html', page], '/app.js': ['text/javascript', script], '/api/inbox': ['application/json', JSON.stringify({ ...receiver.state, actionsEnabled: Boolean(actions), actions: actions?.state })] };
