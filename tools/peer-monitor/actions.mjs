...
export function createExecutor(config) {
  ...
  return async (job, update) => {
    ...
    await run('/bin/sh', [config.actions.wrapper, 'impl', '--track', TRACK, '--slot', slot, '--task-file', taskPath, '--globs', GLOBS.join(','), '--round', String(job.round), '--alarm', '570'], 590000);
    const report = JSON.parse(readFileSync(join(resultDir, `${implId}.json`), 'utf8'));
    validateReport(report); update({ threadId: report.thread_id, files: report.files, engine: report.engine });
