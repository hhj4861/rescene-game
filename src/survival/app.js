...
function sourceRoom() {
  ...
  return `<details id="source-room"><summary>멤버 자료 관리</summary><p class="note">${catalog.personaStatus} 링크만으로 영상을 자동 학습하지 않습니다. 짧게 요약해 등록하고, 직접 검수한 자료만 다음 라운드부터 참고합니다. 첫 제안 전에도 변경할 수 있습니다.</p><p class="note">${state.canManageSources ? '지금 자료를 추가·검수·철회할 수 있습니다.' : '이번 라운드의 자료는 고정됐습니다. 회고 저장 후 변경할 수 있습니다.'}</p><form id="source-form"><fieldset ${state.canManageSources ? '' : 'disabled'}>
