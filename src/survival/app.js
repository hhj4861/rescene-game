...
function render() {
  ...
  if (state.phase === 'voting') actions.push(['vote', '미완료 투표 재시도']);
  ...
  app.innerHTML = `${heading}<main><div class="seasonbar"><strong>Round ${state.round} / 10</strong><div class="rounds">${Array.from({ length: 10 }, (_, i) => `<span class="${i + 1 === state.round ? 'current' : i + 1 < state.round ? 'done' : ''}">${i + 1}</span>`).join('')}</div><span class="note">${state.teams.filter(t => t.alive).length}팀 생존 · ${labels[state.phase]}</span></div>${state.busy ? `<div class="status busy" role="status">${esc(progress || '팀의 응답을 기다리고 있어요')} · 호출 ${r.calls}/55 <button id="cancel" class="secondary small">호출 취소</button></div>` : ''}${state.error ? `<div class="status error" role="alert">${esc(state.error)}</div>` : ''}<div class="layout"><div><section class="room"><h1>${esc(r.concept)}</h1><p>이번 무대의 컨셉입니다. 음악, 안무, 파트를 함께 정해요.</p>${team()}</section><section class="panel"><h2>우리 팀 회의</h2><div class="thread">${posts.length ? posts.map(p => message(p, p.agentId === 'user')).join('') : '<div class="empty">첫 무대에 어떤 이야기를 담을까요?<br>멤버들의 제안부터 들어보세요.</div>'}</div></section>${r.votes.length ? `<div class="vote-list">${[...r.votes, { agentId: 'user', approve: r.userVote }].map(v => `<span class="vote ${v.approve ? 'yes' : 'no'}">${name(v.agentId)} ${v.approve ? '찬성' : '반대'}</span>`).join('')}</div><p class="note">찬성 ${r.votes.filter(v => v.approve).length + Number(r.userVote)}/6 · 4명 이상 동의해야 합니다.${r.needsRediscussion ? ' 나의 반대 의견을 담아 재토론할 기회가 남아 있어요.' : ''}</p>` : ''}${r.evidence.some(e => e.teamId === 'team-0') ? `<section class="panel"><h2>우리의 60초</h2><div class="stage"><div class="beam"></div><div class="beam"></div>${catalog.members.map((m, i) => `<div class="performer" id="performer-${m.id}" data-position="${i}"><span>${m.name}</span></div>`).join('')}</div><div id="cue" class="cue">합의한 파트 순서와 수행 기록을 재생해요.</div><button id="replay" class="secondary small">무대 리플레이</button></section>` : ''}${result(r)}${r.reflections.length ? `<section class="panel"><h2>다음 무대에 가져갈 경험</h2>${r.reflections.map(v => `<div class="source"><strong>${name(v.agentId)}</strong><p>${esc(v.text)}</p><p>다음 행동: ${esc(v.action)}</p>${state.phase === 'learned' ? `<button class="secondary small" data-pin="${v.agentId}">팀 교훈으로 기억하기</button>` : ''}</div>`).join('')}</section>` : ''}${state.phase === 'complete' ? `<section class="room panel"><h2>${esc(state.teams.find(t => t.id === state.champion)?.name)} 우승</h2><p>함께 만든 무대와 경험은 이 시즌에 저장되어 있어요.</p></section>` : ''}</div><aside class="side"><div class="actions">${actions.map(([action, title]) => `<button data-action="${action}" ${can ? '' : 'disabled'}>${title}</button>`).join('')}${state.phase === 'voted' ? `<button id="yes" ${can ? '' : 'disabled'}>나는 찬성 · 투표 시작</button><button id="no" class="secondary" ${can ? '' : 'disabled'}>나는 반대 · 투표 시작</button>` : ''}</div>${planForm(r)}<p id="error" class="inline-error" role="alert"></p><p class="note">${state.provider} · 이번 라운드 ${r.calls}/55회<br>시작은 내가 결정하고, 다음 라운드는 회고 저장 후 열립니다.</p><details><summary>멤버들의 자료와 기억</summary><p class="note">${catalog.personaStatus}</p>${catalog.sources.map(v => `<div class="source"><strong>${name(v.memberId)}</strong> · <a href="${v.url}" target="_blank" rel="noreferrer">${esc(v.title)}</a><br>${esc(v.summary)}</div>`).join('')}${catalog.members.map(m => `<div class="source">${m.name} · 게임 숙련 ${state.members[m.id].skill.toFixed(1)} · 저장 경험 ${state.members[m.id].memories.length}개</div>`).join('')}</details><details><summary>호출 상태 확인</summary>${state.traces.slice(-30).map(t => `<div class="trace">${name(t.agentId)} · ${t.kind} · ${t.status}${t.durationMs ? ` · ${(t.durationMs / 1000).toFixed(1)}초` : ''}<br>${esc(t.sessionId || '응답 대기')}</div>`).join('')}</details><button id="restart" class="secondary small" ${can ? '' : 'disabled'}>새 시즌 준비</button></aside></div><p class="footer">비공식 팬 게임 · 무대, 능력 수치, 심사위원과 대사는 게임 창작입니다. 공개 인터뷰와 게임 경험은 구분해 저장됩니다.</p></main>`;
  ...
  on('restart', async () => { if (confirm('현재 시즌을 마치고 새 시즌을 준비할까요? 기존 시즌 파일은 새 시즌으로 교체됩니다.')) { await post('/api/new', { seed: `우리의 무대 ${Date.now()}`, provider: state.provider }); refresh(true); } });
