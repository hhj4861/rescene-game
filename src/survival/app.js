...
function render() {
  ...
  if (!state) {
    ...
    document.querySelector('#start').onsubmit = async e => { e.preventDefault(); try { await post('/api/new', { seed: document.querySelector('#seed').value, provider: document.querySelector('#provider').value }); refresh(true); } catch (e) { showError(e.message); } }; bindHistory(); return;
  }
