...
function historyRoom() {
  return `<section class="panel" id="history-panel"><h2>지난 시즌</h2><p class="note">보관된 시즌을 읽기 전용으로 봅니다. 현재 시즌은 바뀌지 않습니다.</p><button id="history-load" class="secondary small">시즌 이력 불러오기</button><div id="history-list">${archives.map(s => `<button class="secondary small" data-history="${esc(s.id)}">${esc(s.seed)} · ${s.completedRounds}개 라운드</button>`).join('')}</div>${archiveView ? `<div id="history-view"><h3>${esc(archiveView.seed)}</h3><p>${archiveView.champion ? `우승 ${esc(archiveView.champion)}` : '우승 미확정'}</p>${archiveView.rounds.map(r => `<details><summary>R${r.round} · ${esc(r.concept)} · ${r.ourResult ? `우리 팀 ${r.ourResult.rank}위 / ${r.ourResult.score.toFixed(1)}점 / ${r.ourResult.eliminated ? '탈락' : '생존'}` : '우리 팀 결과 없음'}</summary><ol>${r.ranking.map(t => `<li>${esc(t.name)} · ${t.score.toFixed(1)}점 · ${t.eliminated ? '탈락' : '생존'}</li>`).join('')}</ol></details>`).join('')}</div>` : ''}</section>`;
}
function bindHistory() {
  const get = async path => { const res = await fetch(path), data = await res.json(); if (!res.ok) throw new Error(data.error); return data; };
  document.getElementById('history-load').onclick = async () => {
    try { archives = (await get('/api/history')).seasons; render(); if (!archives.length) document.getElementById('history-list').textContent = '아직 보관된 시즌이 없습니다.'; }
    catch (e) { showError(e.message); }
  };
  document.querySelectorAll('[data-history]').forEach(el => el.onclick = async () => {
    try { archiveView = (await get(`/api/history/${encodeURIComponent(el.dataset.history)}`)).season; render(); }
    catch (e) { showError(e.message); }
  });
}
function roundBrief() {
  const c = state.conceptInfo, o = state.opponent;
  return `${c ? `<p class="note" id="concept-hint">권장 ${c.bpmRange.join('–')} BPM · ${danceNames[c.dance]} · ${esc(c.hint)}<br>${esc(c.parts)}</p>` : ''}${o ? `<article class="source" id="opponent-card"><strong>이번 대진 · ${esc(o.name)}</strong><p>${({ strong: '강', middle: '중', weak: '약' })[o.tier]} 티어 · ${esc(o.hint)} · 직전 ${o.previousScore === null ? '기록 없음' : o.previousScore.toFixed(1) + '점'}</p></article>` : ''}`;
}
function hypothesisView(h) {
  if (!h) return '이전 버전 가설 · 구조화 관찰 없음';
  const label = h.status === 'retired' ? '관찰 종료' : h.status === 'contradicted' ? '반대 관찰' : h.status === 'hypothesis' ? '검증 전 가설' : h.supportedCount >= 2 ? '지지' : '관찰 1회';
  return `${label} · 지지 관찰 ${h.supportedCount || 0}회${h.observations?.length ? `<br>${h.observations.map(o => `R${o.round} 시험됨 · ${esc(o.metric)} ${o.before} → ${o.after} · ${({ supported: '예상 방향', contradicted: '반대 방향', unchanged: '변화 없음' })[o.outcome]}`).join('<br>')}` : ''}`;
}
function growthRoom() {
  ...
  return `<section class="panel growth"><h2>우리의 성장 노트</h2><p class="note">게임 숙련과 경험 기록입니다. 가설의 행동을 실제 수행했는지와 다음 지표 변화를 관찰합니다. 같은 조건 비교 아님 · 단일 라운드 관찰. 두 번의 지지 관찰부터 배지를 표시하며 인과관계를 증명하지 않습니다.</p>${catalog.members.map(m => `<details id="growth-${m.id}"><summary>${m.name} · 숙련 ${state.members[m.id].skill.toFixed(2)} · 피로 ${state.members[m.id].fatigue || 0}/100 · 경험 ${state.members[m.id].memories.length}개</summary>${state.growth[m.id].length ? state.growth[m.id].map(g => {
    ...
    return `<article class="source"><strong>R${g.round} · ${esc(g.concept)}</strong><p>${g.before === undefined ? '이전 버전 기록 · 숙련 전후 수치 없음' : `숙련 ${g.before.toFixed(2)} → ${g.after.toFixed(2)} · 연습 ${g.practice}점`}</p><p>${g.event ? `무대 완성도 ${g.event.quality} · 호흡 부담 ${g.event.breath}` : ''}</p><p>${esc(g.reflection?.text)}</p><p>행동 가설: ${esc(g.reflection?.action)}</p><p>${hypothesisView(g.hypothesis)}</p><p>회복 ${g.recovery || 0}PP · 피로 ${g.fatigueBefore || 0} → ${g.fatigueAfter || 0}</p><p>${g.referencedBy.length ? `제안에서 참고: ${g.referencedBy.map(n => `R${n}`).join(', ')} (효과 입증과 다릅니다)` : '다음 제안 반영 기록 없음'}</p>${excluded ? '<p class="out">철회 자료가 연관되어 이후 기억 검색에서 제외됩니다. 과거 경기 기록은 보존합니다.</p>' : ''}</article>`;
...
function render() {
  ...
  document.querySelectorAll('[data-recovery]').forEach(el => el.oninput = () => { draft.recovery[Number(el.dataset.recovery)] = Number(el.value); });
  ...
  on('discuss', () => { const { message, ...plan } = draft; if (!r.rulesVersion) delete plan.recovery; command('discuss', { plan, message: message || plan.direction }); });
  ...
  on('listen', playMusic);
  mountGame({ app, state, catalog, draft });
}
function playMusic() {
