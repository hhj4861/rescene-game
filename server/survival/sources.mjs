...
export function addSource(library, input) {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(publishedAt) || !Number.isFinite(Date.parse(publishedAt)) || new Date(publishedAt).toISOString().slice(0, 10) !== publishedAt || publishedAt > new Date().toISOString().slice(0, 10)) throw new Error('실제 발행일을 입력하세요');
